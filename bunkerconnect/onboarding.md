...
Deze pagina beschrijft hoe organisaties (bunker suppliers) deelnemer worden van BunkerConnect — van self-service registratie via verificatie tot goedkeuring. Na goedkeuring is de organisatie een actieve participant in het BunkerConnect Organization Registry (OR) en kan zij een applicatie aansluiten om policies te registreren.

> 🔗 **Self-Service Portal:** [bunkerconnect-preview.poort8.nl/portal ➚](https://bunkerconnect-preview.poort8.nl/portal)
...
flowchart LR
    ...
    C --> D[BunkerConnect-beheerder beoordeelt & keurt goed]
    D --> E[Organisatie is actief]
```

## Self-service registratie

Elke gebruiker kan zijn organisatie registreren via de [Self-Service Portal](https://bunkerconnect-preview.poort8.nl/portal). De registrerende gebruiker:
...
Na registratie staat de organisatie ter beoordeling voor goedkeuring door de BunkerConnect-beheerder.
...
| **Goedkeuring onboarding** | Dataspace-beheerder (BunkerConnect) | Definitieve beslissing over deelname | Goedgekeurd, afgewezen of ingetrokken |

## Goedkeuringsproces

Na registratie beoordeelt de BunkerConnect-beheerder de gegevens van de organisatie en keurt deelname goed of af. Tot de organisatie is goedgekeurd, kunnen gebruikers van die organisatie nog geen dataspace-systemen gebruiken.

## Organisatie-identifiers

Elke organisatie in BunkerConnect wordt geïdentificeerd via een EUID (European Unique Identifier), conform de NoodleBar-standaard:
