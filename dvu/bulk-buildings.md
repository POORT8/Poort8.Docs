...
The bulk building flow works the same as [Single Building Access](single-building.md), but accepts an array of addresses instead of a single address. The energy contractor approves all buildings in one approval link. Upon approval, Keyper registers a separate policy per building in DVU.
...
sequenceDiagram
    ...
    Keyper->>DVU: Redirect to metadata app (bulk)
    EC->>DVU: Review and complete building details
