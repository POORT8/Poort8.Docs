...
This guide is for **David** — a data service consumer who wants to call APIs registered in the dataspace. It covers the full process: registering your application, requesting access to an API, obtaining access tokens, and calling the API.
...
```
POST https://auth.poort8.nl/realms/{realm}/protocol/openid-connect/token
```

> Replace `{realm}` with the value provided by your dataspace administrator.
...
```bash
curl -X POST https://auth.poort8.nl/realms/{realm}/protocol/openid-connect/token \
...
var response = await httpClient.PostAsync(
    "https://auth.poort8.nl/realms/{realm}/protocol/openid-connect/token",
...
| Claim | Description |
|-------|-------------|
| `iss` | Token issuer — the Association Registry |
...
The Association Registry publishes its configuration at the standard OpenID Connect discovery endpoint:
...
Your dataspace administrator will provide the specific URLs for your instance.

| Resource | URL Pattern |
|----------|-------------|
| **Self-Service Portal** | `https://{domain}.poort8.nl/portal` |
| **Token endpoint** | `https://auth.poort8.nl/realms/{realm}/protocol/openid-connect/token` |
