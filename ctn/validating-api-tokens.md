...
This guide is for **Charlie** — a data service provider who exposes APIs in the CTN dataspace. It covers how to register your API, manage access requests, and validate the JWT access tokens that consumers present when calling your API.
...
sequenceDiagram
    ...
    David->>CTN: Request token (client_credentials)
    CTN-->>David: Signed JWT access token
...
Every API request from a CTN consumer includes a JWT access token in the `Authorization` header. You **must** validate this token before processing the request.
...
|---|-------|---------------|------------|
| 1 | **Signature** | JWT signature is valid against the CTN Association Registry's public keys (JWKS) | `401 Unauthorized` |
| 2 | **Expiration** | `exp` claim is in the future | `401 Unauthorized` |
| 3 | **Issuer** | `iss` claim equals `https://auth.poort8.nl/realms/ctn-preview` | `401 Unauthorized` |
...
The CTN Association Registry publishes its signing keys at:

```
https://auth.poort8.nl/realms/ctn-preview/protocol/openid-connect/certs
```
...
A decoded access token from a CTN consumer looks like this:
...
|-------|------|-------------|
| `iss` | string | Token issuer — must be the CTN Association Registry |
...
The `organization` claim identifies the consumer's organization in the CTN dataspace:
...
The key of the object (e.g., `NLNHR.12345678`) is the organization's **EUID** — the same identifier that was assigned and verified during registration with the CTN Association Registry. This is not a self-declared value: the EUID is derived from the organization's official business registration number (KvK, KBO, or court registration), verified by the CTN administrator before the organization was approved.
...
const jwtCheck = auth({
  issuerBaseURL: 'https://auth.poort8.nl/realms/ctn-preview',
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    ...
    {
        options.Authority = "https://auth.poort8.nl/realms/ctn-preview";
...
ISSUER = "https://auth.poort8.nl/realms/ctn-preview"
AUDIENCE = "your-api-client-id"
...
The CTN Association Registry publishes its full configuration (issuer, endpoints, signing algorithms, supported claims) at:
...
| **JWKS endpoint** | `https://auth.poort8.nl/realms/ctn-preview/protocol/openid-connect/certs` |
| **OIDC Discovery** | `https://auth.poort8.nl/realms/ctn-preview/.well-known/openid-configuration` |
| **Expected issuer** | `https://auth.poort8.nl/realms/ctn-preview` |
